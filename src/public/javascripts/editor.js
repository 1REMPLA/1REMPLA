	...
	var methods = {
			...
			var tblCntr = $('<div/>',{ //Outer Container Div
				class:"row-fluid"
					...
					id:"tblForm"+idExtn
						...
						class:"row"
						...
						}).append($('<label/>',{ for:"tblRows"+idExtn,	text:"Lignes"}
							)).append($('<input/>',{
								id:"tblRows"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:2
							})).append($('<label/>',{ for:"tblColumns"+idExtn,	text:"Colonnes"}
							)).append($('<input/>',{
								id:"tblColumns"+idExtn,
								type:"text",
							 	class:"form-control form-control-width",
							 	value:2
							})).append($('<label/>',{ for:"tblWidth"+idExtn, text:"Largeur"}
							)).append($('<input/>',{
								id:"tblWidth"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:400
							})).append($('<label/>',{ for:"tblHeight"+idExtn, text:"Hauteur"}
							)).append($('<input/>',{
								id:"tblHeight"+idExtn,
								type:"text",
								class:"form-control form-control-width",
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
						}).append($('<label/>',{ for:"tblAlign"+idExtn, text:"Alignement"}
							)).append($('<select/>',{ id:"tblAlign"+idExtn, class:"form-control form-control-width"}
						).append($('<option/>',{ text:"Choisir", value:""}
					)).append($('<option/>',{ text:"Gauche", value:"left"}
				)).append($('<option/>',{ text:"Centre", value:"center"}
			)).append($('<option/>',{ text:"Droite",	value:"right"}))
		).append($('<label/>',{	for:"tblBorder"+idExtn, text:"Taille des Bords"}
							)).append($('<input/>',{
								id:"tblBorder"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"Espacement des cellules"}
							)).append($('<input/>',{
								id:"tblCellspacing"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"Distance des cellules"}
							)).append($('<input/>',{
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
						     ...
						     'ol'		: { "text": "N",
											"icon": "fa fa-list-ol",
											"tooltip": "Ajouter/Supprimer Liste numérotée",
											"commandname":"insertorderedlist",
											"custom":null },

						     'ul'		: { "text": "Bullet",
											"icon": "fa fa-list-ul",
											"tooltip": "Ajouter/Supprimer Liste",
						...
						'insert_table'	: { "modal": true,
											...
											"modalHeader": "Insérer un tableau",
						   ...
						   'indent'		: { "text": "Indent",
											"icon":"fa fa-indent",
											"tooltip": "Décaler vers la Droite",
											"commandname":"indent",
											"custom":null },

						   'outdent'	: { "text": "Outdent",
											"icon":"fa fa-outdent",
											"tooltip": "Décaler vers la Gauche",
											"commandname":"outdent",
											"custom":null },
							...
							'splchars'	: { "text": "S",
											"icon": "fa fa-asterisk",
											"tooltip": "Insérer un caractère spécial",
											"commandname":null,
											"custom":function(button){
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").append('<div class="label">'+'Lettres : '+charCount+'</div>');
            	});
	        }
								            	...
								            	class:"modal-dialog"
							            			...
							            			class:"modal-content"
									            		...
									            		class:"modal-footer"
								                			...
								                			type:"button",
								                			class:"btn btn-success",
																		}).html('Insérer').mousedown(function(e){
								                			e.preventDefault();
...
function getTText() {
	...
	// 	bio = $("#txtEditor").getText();
	// });
  // var bio = document.getElementById("txtEditor").getText();
	document.getElementById("biographie").value = ($('#txtEditor').Editor('getText'));
}
